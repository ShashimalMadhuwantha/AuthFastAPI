...
let customDateRange = null; // Will store {startDate, endDate} if custom range is set
...
// Get cutoff time based on current settings
function getCutoffTime() {
    if (customDateRange) {
        return new Date(customDateRange.startDate);
    }
    return new Date(Date.now() - timePeriodHours * 60 * 60 * 1000);
}

// Get end time based on current settings
function getEndTime() {
    if (customDateRange) {
...
function filterDataByTimeRange() {
    ...
    Object.keys(devicesData).forEach(deviceId => {
        Object.keys(devicesData[deviceId].sensors).forEach(sensorType => {
            ...
            sensor.values = sensor.values.filter(v => {
            ...
            if (sensor.values.length > 0) {
                sensor.min = Math.min(...sensor.values.map(v => v.value));
                sensor.max = Math.max(...sensor.values.map(v => v.value));
                sensor.sum = sensor.values.reduce((sum, v) => sum + v.value, 0);
                sensor.count = sensor.values.length;
            } else {
                sensor.min = Infinity;
                sensor.max = -Infinity;
                sensor.sum = 0;
...
function updateSensorData(deviceId, sensorType, data) {
    ...
    // Remove data outside the selected time range
    const cutoffTime = getCutoffTime();
    const endTime = getEndTime();
    sensor.values = sensor.values.filter(v => {
        const ts = new Date(v.timestamp);
        return ts >= cutoffTime && ts <= endTime;
    });
